    ...
    const { data: issues = [], isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            const res = await axios.get(
                `http://localhost:5000/myissue?email=${user?.email}`
            );
            return res.data;
        },
    });
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const res = await axios.delete(`http://localhost:5000/issue/${id}`);
                    ...
                    <tbody>
                        {issues.map((issue) => (
                            ...
                            >
                                ...
                                <td
                                    className={`py-3 px-4 font-bold ${issue.status === "ongoing"
                                        ? "text-red-500"
                                        : "text-green-600"
                                        }`}
                                >
